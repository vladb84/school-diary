...
// GC: for grade cells in GradesTab (subject table, HW list)
const GC={"5":"gc-5","4":"gc-4","3":"gc-3","2":"gc-2"};
// gcl: returns CSS class for grade cell in detailed stats
...
export default function GradesTab({
    ...
    return(
      <div key={id} style={{marginBottom:"12px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
          <button onClick={toggle} style={{flex:1,display:"flex",alignItems:"center",justifyContent:"space-between",background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:collapsed?"14px":"14px 14px 0 0",padding:"10px 14px",cursor:"pointer",textAlign:"left"}}>
            <span style={{fontSize:"13px",fontWeight:"500",color:"var(--text-primary)"}}>{title}</span>
            <span style={{fontSize:"14px",color:"var(--text-muted)",display:"inline-block",transform:collapsed?"rotate(-90deg)":"rotate(0deg)",transition:"transform 0.15s"}}>⌄</span>
          </button>
  ...
  if (tab === 2) {
    return (
      <div>
        {isOwner&&<p className="text-xs mb-3 text-center" style={{color:"var(--text-muted)"}}>Нажмите на оценку для изменения · на предмет — для статистики</p>}
        <div className="space-y-3 mb-4">
          {schSubjs.some(s=>sjGrades(s.id).length>0)
            ?schSubjs.filter(s=>sjGrades(s.id).length>0).map(s=>{
              ...
              return(
                <Card key={s.id} cls="cursor-pointer transition-all" onClick={()=>{setSelSubj(s.id);setTab(3);}}>
        ...
        {isOwner&&(
          <Card>
            ...
            {showAddGrade&&(
              schSubjs.length===0
                ?<p className="text-xs text-center py-2 mt-3" style={{color:"var(--text-muted)"}}>Сначала добавьте предметы в расписание</p>
  ...
  if (selSubj) {
    ...
    return(
      <div>
        <button onClick={()=>setSelSubj(null)} className="text-sm flex items-center gap-1 mb-4" style={{color:"var(--text-muted)"}}>← Все предметы</button>
        <div style={{background:color,borderRadius:"18px",padding:"20px",marginBottom:"14px",color:"#fff"}}>
        ...
        {sorted.length>0&&(
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:"14px",padding:"14px",marginBottom:"14px"}}>
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px"}}>
              <p style={{fontSize:"13px",fontWeight:"500",margin:"0",color:"var(--text-primary)"}}>Оценки</p>
              {avg2&&<div style={{display:"flex",alignItems:"center",gap:"6px"}}>
                <span style={{fontSize:"11px",color:"var(--text-muted)"}}>среднее</span>
                <span className={gcl(String(Math.round(parseFloat(avg2))))} style={{fontSize:"13px",fontWeight:"500",padding:"2px 6px",borderRadius:"6px"}}>{avg2}</span>
        ...
        {gs.length===0&&<div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:"14px",padding:"14px",marginBottom:"14px"}}><p className="text-sm text-center py-2" style={{color:"var(--text-muted)"}}>Оценок пока нет</p></div>}
        {hw.length>0&&(
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderRadius:"14px",overflow:"hidden"}}>
            <button onClick={()=>setShowHw(v=>!v)} style={{width:"100%",display:"flex",alignItems:"center",justifyContent:"space-between",padding:"14px",cursor:"pointer",background:"none",border:"none",textAlign:"left"}}>
              <span style={{fontSize:"13px",fontWeight:"500",color:"var(--text-primary)"}}>Домашние задания ({hw.length})</span>
              <span style={{fontSize:"11px",color:"var(--text-muted)",transform:showHw?"rotate(0deg)":"rotate(-90deg)",transition:"transform 0.15s",display:"inline-block"}}>⌄</span>
            </button>
            {showHw&&(
              <div className="space-y-2" style={{padding:"0 14px 14px"}}>
                {hw.map(h=>(
                  <div key={h.id} className={`flex items-start gap-2 p-2.5 rounded-xl ${h.done?"opacity-60":""}`} style={{background:"var(--bg-card-done)"}}>
                    <span className={`mt-0.5 w-4 h-4 rounded-full border-2 flex-shrink-0 flex items-center justify-center text-xs ${h.done?"bg-green-500 border-green-500 text-white":""}`} style={h.done?{}:{borderColor:"var(--border)"}}>{h.done&&"✓"}</span>
                    <div className="flex-1 min-w-0">
                      <p className={`text-xs ${h.done?"line-through":""}`} style={{color:"var(--text-secondary)"}}>{h.task}</p>
                      {h.date&&<p className="text-xs mt-0.5" style={{color:"var(--text-muted)"}}>до {fmtDate(h.date)}</p>}
                    </div>
      ...
      <div style={{display:"flex",gap:"6px",flexWrap:"wrap"}}>
        {lg.map((g,i)=>{const cl=gcl(g.value);return(
          <div key={i} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:"3px"}}>
            <div className={cl} style={{width:"36px",height:"36px",borderRadius:"10px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",border:isKR(g.type)?"2px solid currentColor":"2px solid transparent",boxSizing:"border-box"}}>
              <span style={{fontSize:"9px",lineHeight:"1"}}>{gradeIcon(g.type)}</span>
              <span style={{fontSize:"14px",fontWeight:"500",lineHeight:"1.1"}}>{g.value}</span>
            </div>
            <div style={{fontSize:"10px",color:"var(--text-muted)"}}>{g.sn?.split(" ")[0]?.slice(0,4)}</div>
          </div>
        );})}
      </div>
    ),
      ...
      <div>
        {ss.map(({s,a,t})=>(
          <div key={s.id} onClick={()=>setSelSubj(s.id)} style={{display:"flex",alignItems:"center",gap:"8px",marginBottom:"10px",cursor:"pointer",padding:"4px",borderRadius:"8px"}}>
            <span style={{fontSize:"12px",flex:"1",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"var(--text-primary)"}}>{s.name}</span>
      ...
      <div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"10px"}}>
          <div style={{background:"var(--bg-card-done)",borderRadius:"10px",padding:"10px",textAlign:"center"}}>
            <div style={{fontSize:"22px",fontWeight:"500",color:"var(--text-primary)"}}>{wT>0?wp+"%":"—"}</div>
            <div style={{fontSize:"11px",color:"var(--text-muted)"}}>эта неделя</div>
            {wT>0&&<div style={{fontSize:"11px",color:"var(--text-muted)"}}>{wD} из {wT}</div>}
          </div>
          <div style={{background:"var(--bg-card-done)",borderRadius:"10px",padding:"10px",textAlign:"center"}}>
            <div style={{fontSize:"22px",fontWeight:"500"}}>{aT>0?ap+"%":"—"}</div>
            <div style={{fontSize:"11px",color:"var(--text-muted)"}}>всего</div>
            {aT>0&&<div style={{fontSize:"11px",color:"var(--text-muted)"}}>{aD} из {aT}</div>}
          </div>
        </div>
      ...
      {ss.length>1&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px",marginBottom:"12px"}}>
          ...
          <div style={{background:"var(--bg-card)",border:"1px solid var(--border)",borderLeft:"3px solid var(--danger)",borderRadius:"0 14px 14px 0",padding:"14px"}}>
            ...
            <p style={{fontSize:"13px",fontWeight:"500",margin:"0 0 5px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",color:"var(--text-primary)"}}>{worst.s.name}</p>
